...
contract Coder {
    ...
    function abiEncodeStr(string memory str) public pure returns(bytes memory bts){
        return abi.encode(str);
    }
    
    function abiDecodeStr(bytes memory bts) public pure returns(string memory str){
        return abi.decode(bts, (string));
    }
    
    function selector(string memory method) public pure returns(bytes4 _selector) {
        return bytes4(keccak256(bytes(method)));
    }
    
    function encodeTxArgs(bytes memory toAssetHash,bytes memory toAddress,uint256 amount) public pure returns(bytes memory){
        TxArgs memory t = TxArgs(
            toAssetHash,
            toAddress,
            amount
        );
        return _serializeTxArgs(t);
    }
    
    function decodeTxArgs(bytes memory v) public pure returns(bytes memory toAssetHash,bytes memory toAddress,uint256 amount){
        TxArgs memory t = _deserializeTxArgs(v);
        return (t.toAssetHash,t.toAddress,t.amount);
    }
    
    function encodeSwapArgs(
        uint amount, uint minOut,
        uint64 toPoolId,uint64 toChainId,
        bytes memory fromAssetHash,bytes memory fromAddress,
        bytes memory toAssetHash,bytes memory toAddress)
        public pure
        returns(bytes memory)
    {
        SwapArgs memory t = SwapArgs(
            amount,
            minOut,
            toPoolId,
            toChainId,
        ...
        return _serializeSwapArgs(t);
    }
    
    function decodeSwapArgs(bytes memory v) public pure
    returns(uint amount, uint minOut,uint64 toPoolId,uint64 toChainId,
            bytes memory fromAssetHash,bytes memory fromAddress,
            bytes memory toAssetHash,bytes memory toAddress){
        SwapArgs memory t = _deserializeSwapArgs(v);
        return (t.amount,t.minOut,t.toPoolId,t.toChainId,t.fromAssetHash,t.fromAddress,t.toAssetHash,t.toAddress);
    }
    
    function encodeCcmArgs(
        bytes memory txHash,bytes memory crossChainId,
        bytes memory fromContract,uint64 toChainId,
        bytes memory toContract,bytes memory method,bytes memory args)
        public pure 
        returns(bytes memory) 
    {
        CCMArgs memory t = CCMArgs(
            txHash,
            crossChainId,
            fromContract,
            toChainId,
        ...
        return _serializeCCMArgs(t);
    }
    
    function decodeCcmArgs(bytes memory v) public pure 
    returns(bytes memory txHash,bytes memory crossChainId,
            bytes memory fromContract,uint64 toChainId,
            bytes memory toContract,bytes memory method,bytes memory args){
        CCMArgs memory t = _deserializeCCMArgs(v);
        return (t.txHash,t.crossChainId,t.fromContract,t.toChainId,t.toContract,t.method,t.args);
    }
    ...
    function _serializeSwapArgs(SwapArgs memory args) internal pure returns (bytes memory) {
        bytes memory buff;
        buff = abi.encodePacked(
            ZeroCopySink.WriteUint255(args.amount),
    ...
    function _serializeCCMArgs(CCMArgs memory args) internal pure returns (bytes memory) {
        bytes memory buff;
        buff = abi.encodePacked(
            ZeroCopySink.WriteVarBytes(args.txHash),
            ZeroCopySink.WriteVarBytes(args.crossChainId),
            ZeroCopySink.WriteVarBytes(args.fromContract),
            ZeroCopySink.WriteUint64(args.toChainId),
            ZeroCopySink.WriteVarBytes(args.toContract),
            ZeroCopySink.WriteVarBytes(args.method),
            ZeroCopySink.WriteVarBytes(args.args)
            );
        return buff;
    }
    
    function _deserializeCCMArgs(bytes memory _valueBs) internal pure returns (CCMArgs memory) {
        CCMArgs memory txParam;
        uint256 off = 0;
